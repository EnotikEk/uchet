...
def init_db():
    ...
    if not table_exists:
        # Создаем новую таблицу с колонкой mol_employee
        cursor.execute("""
        ...
        """)
        print("✅ Таблица Equipment создана с колонкой mol_employee")
    else:
        # Проверяем наличие колонки mol_employee в существующей таблице
        cursor.execute("PRAGMA table_info(Equipment)")
        existing_columns = [col[1] for col in cursor.fetchall()]
        
        if 'mol_employee' not in existing_columns:
            print("➕ Добавление колонки mol_employee в существующую таблицу...")
            cursor.execute("ALTER TABLE Equipment ADD COLUMN mol_employee INTEGER")
            print("✅ Колонка mol_employee добавлена")
        else:
            print("✅ Колонка mol_employee уже существует")
