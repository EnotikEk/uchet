...
def add_equipment():
    ...
    try:
        ...
        print(f"DEBUG: Филиал для нового оборудования: {org_id}")
        ...
        if inventory_number:
            ...
            if existing:
                ...
                return jsonify({
                    'success': False, 
                    'error': f'Оборудование с инвентарным номером "{inventory_number}" уже существует в вашем филиале!'
                }), 400
        # Если номер не указан - просто сохраняем NULL, никаких проверок не делаем
        ...
        cursor.execute("""
            INSERT INTO Equipment (
                ...
                monitor_size, resolution, refresh_rate, panel_type, response_time, ports,
        ...
        """, (
            ...
            data.get('panel_type'), data.get('response_time'), data.get('ports'),
...
def get_equipment():
    ...
    cursor.execute(f"""
        SELECT 
            ...
            e.monitor_size, e.resolution, e.refresh_rate, e.panel_type, e.response_time, e.ports,
            ...
            rm.Number as room,
            d.name as department_name,   -- отдел из Departments по office
        ...
        LEFT JOIN Departments d ON rm.Number = d.office   -- <--- ИСПРАВЛЕНО
    ...
    for row in cursor.fetchall():
        equipment.append({
            ...
            'ports': row[17] or '',
            'port_count': row[18] or '',
            'speed': row[19] or '',
            'network_type': row[20] or '',
            'poe': row[21] or '',
            'managed': row[22] or '',
            'ip_address': row[23] or '',
            'print_type': row[24] or '',
            'print_format': row[25] or '',
            'print_speed': row[26] or '',
            'color_type': row[27] or '',
            'duplex': row[28] or '',
            'printer_ports': row[29] or '',
            'scanner_resolution': row[30] or '',
            'scanner_speed': row[31] or '',
            'phone_number': row[32] or '',
            'phone_ip': row[33] or '',
            'sip_account': row[34] or '',
            'lines': row[35] or '',
            'phone_poe': row[36] or '',
            'ups_power': row[37] or '',
            'ups_type': row[38] or '',
            'ups_outlets': row[39] or '',
            'ups_usb': row[40] or '',
            'ups_runtime': row[41] or '',
            'connection_type': row[42] or '',
            'color': row[43] or '',
            'is_set': row[44] if len(row) > 44 else 0,
            'set_type': row[45] if len(row) > 45 else '',
            'responsible': row[46] if len(row) > 46 else '',
            'mol': row[47] if len(row) > 47 else '',
            'room': row[48] if len(row) > 48 else '',
            'department_name': row[49] if len(row) > 49 else '',
            'organization': row[50] if len(row) > 50 else '',
            'organization_id': row[51] if len(row) > 51 else None,
            'status': row[52] if len(row) > 52 else 'В работе',
            'purchase_date': row[53] if len(row) > 53 and row[53] else '',
            'warranty_until': row[54] if len(row) > 54 and row[54] else '',
            'price': float(row[55]) if len(row) > 55 and row[55] else 0,
            'supplier': row[56] if len(row) > 56 else '',
            'notes': row[57] if len(row) > 57 else '',
            'created_at': row[58] if len(row) > 58 else None,
            'created_by': row[59] if len(row) > 59 else row[60] if len(row) > 60 else ''
        })
